...
在 `settings.py` 中设置 `DJANGO_COOL` ,详情查看 :doc:`/settings`
...
自定义 `Model` 继承 :class:`~cool.model.BaseModel` 可使用扩展功能：

+ 支持字段变更监控记录
    - 通过 :func:`~cool.model.BaseModel.save_changed()` 保存已修改字段

+ 主键唯一键缓存
    - 缓存获取： :func:`~cool.model.BaseModel.get_obj_by_pk_from_cache()` :func:`~cool.model.BaseModel.get_obj_by_unique_key_from_cache()`
    - 删除缓存： :func:`~cool.model.BaseModel.flush_cache_by_pk()` :func:`~cool.model.BaseModel.flush_cache_by_unique_key()` :func:`~cool.model.BaseModel.flush_cache()`

+ 搜索字段自动生成
    - :func:`~cool.model.BaseModel.get_search_fields()` 自动生成搜索字段，默认返回所有设置索引的char和int类型字段

后台管理扩展
--------------------

:class:`~cool.admin.BaseModelAdmin` 提供扩展功能：
...
使用 :func:`~cool.admin.admin_register()` 装饰器可以快速将 `Model` 注册到后台管理
...
+ :class:`~cool.views.CoolBFFAPIView` 可方便创建 ``application/x-www-form-urlencoded`` / ``multipart/form-data`` 方式的接口。
+ `Meta` 类中配置参数列表 `param_fields` 后可以自动生成接口文档，自动做参数验证
+ 使用 :class:`~cool.views.ViewSite` 快速注册接口生成 `urlpatterns`

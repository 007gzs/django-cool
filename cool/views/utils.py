...
def get_rest_field_from_model_field(model, model_field, **kwargs):
    ...
    def _check_required_default():
        if 'default' in kwargs or 'required' in kwargs:
            return
        if field_kwargs.get('required') or field_kwargs['default'] not in (None, ""):
            return
        if field_kwargs.get('allow_null', False) and field_kwargs['default'] is None:
            return
        if field_kwargs.get('allow_blank', False) and field_kwargs['default'] == "":
            return
        field_kwargs['required'] = True
        field_kwargs.pop('default')

    _check_required_default()
